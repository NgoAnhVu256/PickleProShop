...
export default function ProductCard({ product }: { product: any }) {
  ...
  return (
    <Link href={`/products/${product.slug}`} className="bg-white p-2 md:p-3 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow group flex flex-col gap-2 md:gap-3 h-full">
      ...
      <button className="w-full py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-[10px] md:text-xs font-black hover:bg-[#7DAACB] hover:text-white hover:border-[#7DAACB] transition-colors uppercase tracking-widest mt-1">
        Xem chi tiết
      </button>
    </Link>
  );
