...
import { ArrowLeft, Save, Plus, Trash2, Tag, Layers, Loader2, DollarSign, Image as ImageIcon, X } from "lucide-react";
    ...
    const fetchData = async () => {
      try {
        const [pRes, cRes, bRes] = await Promise.all([
          fetch(`/api/admin/products/${id}`), fetch("/api/admin/categories"), fetch("/api/admin/brands")
        ]);
        ...
        if (pData.success) {
          ...
          setBasePrice(String(p.basePrice)); setSalePrice(p.salePrice ? String(p.salePrice) : "");
      ...
      } catch { toast.error("Lỗi đồng bộ dữ liệu"); }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/admin/products/${id}`, {
        ...
        body: JSON.stringify({
          name, description, basePrice: bp || 0, salePrice: salePrice ? sp : null,
      ...
      if (data.success) { toast.success("Cập nhật thành công!"); router.push("/admin/products"); }
        ...
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 24 }} className="admin-grid-layout">
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div className="card" style={{ padding: 24 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                  <div><label className="input-label">Thương hiệu</label><select className="input" value={brandId} onChange={e => setBrandId(e.target.value)}><option value="">Không có</option>{brands.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}</select></div>
                  <div><label className="input-label">Danh mục *</label><select className="input" value={categoryId} onChange={e => setCategoryId(e.target.value)} required>{categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
                </div>
              </div>
            </div>
            ...
            <div className="card" style={{ padding: 24 }}>
              ...
              <Suspense fallback={<div className="skeleton" style={{ height: 300 }} />}><RichTextEditor value={description} onChange={setDescription} /></Suspense>
            </div>
            ...
            <div className="card" style={{ padding: 24 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
                <div><label className="input-label">Giá gốc (VNĐ) *</label><input className="input" type="number" value={basePrice} onChange={e => setBasePrice(e.target.value)} min="0" /></div>
                <div>
            ...
            <div className="card" style={{ padding: 24 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 24 }}>
                <ImageUpload label="Thumbnail" value={thumbnail} onChange={setThumbnail} onRemove={() => setThumbnail("")} folder="products" />
                <MultiImageUpload label="Gallery" value={galleryImages} onChange={setGalleryImages} onRemove={url => setGalleryImages(galleryImages.filter(i => i !== url))} folder="products" />
              </div>
            </div>
