...
import { Plus, Trash2, ArrowLeft, Tag, Layers, Loader2, DollarSign, Image as ImageIcon } from "lucide-react";
        ...
        body: JSON.stringify({
          ...
          variants: variants
            ...
            .map((v) => ({
              ...
              images: v.images,
              attrValues: v.attrValues.filter(av => av.value)
            })),
        }),
      });
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div className="card" style={{ padding: 24 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                <h2 className="section-title" style={{ marginBottom: 0 }}><Layers size={18} color="#58d68d" /> Biến thể (Mẫu mã / Màu sắc)</h2>
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16, marginBottom: 16 }}>
                {variants.map((variant, vIndex) => (
                  <div key={vIndex} style={{ padding: 16, border: "1px solid #eef2f7", borderRadius: 12, background: "#f8f9fb" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                      <span style={{ fontSize: 13, fontWeight: 700, color: "#5a6677" }}>Biến thể #{vIndex + 1}</span>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 16 }}>
                      <div>
                        <label className="input-label" style={{ fontSize: 11 }}>SKU *</label>
                        <input className="input" value={variant.sku} onChange={(e) => updateVariant(vIndex, "sku", e.target.value)} placeholder="SKU..." style={{ background: "#fff" }} />
                      </div>
                      <div>
                        <label className="input-label" style={{ fontSize: 11 }}>Giá (VNĐ)</label>
                        <input className="input" type="number" value={variant.price} onChange={(e) => updateVariant(vIndex, "price", e.target.value)} style={{ background: "#fff" }} />
                      </div>
                      <div>
                        <label className="input-label" style={{ fontSize: 11 }}>Tồn kho</label>
                        <input className="input" type="number" value={variant.stock} onChange={(e) => updateVariant(vIndex, "stock", e.target.value)} style={{ background: "#fff" }} />
                      </div>
                    </div>
                    ...
                    {categoryAttributes.length > 0 && (
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, paddingTop: 12, borderTop: "1px solid #eef2f7" }}>
