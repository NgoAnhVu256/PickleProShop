        ...
        if (d.success) {
          ...
          const conds = promo.conditions.map((c: any) => {
            if (c.categoryId && c.category) {
              return { type: "category", categoryId: c.category.id, name: `Danh mục: ${c.category.name}` };
            } else if (c.productVariantId && c.productVariant) {
              ...
              return { type: "variant", productVariantId: pv.id, name: fullName, sku: pv.sku, image: pv.product.thumbnail };
            }
            return null;
          }).filter(Boolean);
          setConditions(conds);

          const rews = promo.rewards.map((r: any) => {
  ...
  return (
    <div className="fade-in pb-20">
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
        <Link href="/admin/promotions" style={{ color: "#8a98ac" }}><ArrowLeft size={20} /></Link>
        <h1 style={{ fontSize: 24, fontWeight: 800, color: "#323b4b" }}>Chi Tiết & Cập Nhật Combo Khuyến Mãi</h1>
      </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 24 }}>
          ...
          <div style={{ display: "flex", gap: 16, marginTop: 16 }}>
            <button type="submit" className="btn btn-primary" style={{ padding: "14px 32px", fontSize: 15 }} disabled={submitting}>
              ...
              {submitting ? "Đang xử lý..." : "Cập Nhật Chương Trình"}
            </button>
            <Link href="/admin/promotions" className="btn btn-secondary" style={{ padding: "14px 32px", fontSize: 15, textDecoration: "none" }}>Hủy</Link>
          </div>
