...
function Sidebar({ user }: { user: any }) {
  ...
  const avatarUrl = user?.image || `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.name || "U")}&background=2C2877&color=fff&size=128&bold=true`;
  ...
  return (
    <aside className="w-full lg:w-[260px] shrink-0">
      <div className="flex items-center gap-3 mb-6">
        <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-primary/20 bg-white shadow-sm">
      ...
      <nav className="space-y-1 mb-4">
        {navItems.map(item => {
          ...
          return (
            ...
            >
              <Icon size={18} className={item.active ? "text-primary" : "text-gray-400"} />
              {item.label}
            </Link>
          );
        })}
            ...
            {filteredOrders.length === 0 ? (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-12 text-center">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  <Link href="/products" className="px-4 py-2 bg-white border border-gray-200 rounded-full text-sm font-bold text-gray-700 hover:border-primary transition-colors">
                    Sản phẩm
                  </Link>
                  <Link href="/" className="px-4 py-2 bg-white border border-gray-200 rounded-full text-sm font-bold text-gray-700 hover:border-primary transition-colors">
                    ← Về trang chủ
                  </Link>
                </div>
              </div>
                    ...
                    <div key={order.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                      ...
                      <div className="border-b border-gray-100 px-5 py-4 flex flex-col sm:flex-row justify-between gap-3 bg-gray-50/50">
                        <div>
                          <p className="text-sm font-bold text-gray-900">
                            Mã đơn hàng: <span className="text-primary">#{order.id.slice(-6).toUpperCase()}</span>
                          </p>
                            ...
                            return (
                              <div key={item.id} className="flex gap-4 items-center">
                                ...
                                <div className="flex-1 min-w-0">
                                  <Link href={`/products/${product.slug}`} className="text-sm font-bold text-gray-900 hover:text-primary line-clamp-1">
                                    {product.name}
                                  </Link>
                                  {attrText && <p className="text-xs text-gray-500 mt-0.5">{attrText}</p>}
                                  <div className="flex items-center gap-4 mt-1">
                                    <p className="text-sm font-semibold text-accent">{item.price.toLocaleString()}₫</p>
                      ...
                      <div className="px-5 py-4 bg-gray-50/30 flex flex-col md:flex-row gap-4 justify-between">
                        ...
                        <div className="flex flex-col items-end justify-center min-w-[180px] border-t md:border-t-0 md:border-l border-gray-200 pt-3 md:pt-0 md:pl-6">
                          ...
                          <p className="text-xl font-black text-accent">{order.totalPrice.toLocaleString()}₫</p>
                        </div>
                      </div>
                    </div>
                  );
